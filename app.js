document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadUserAirdrops = () => {
        airdrops = AppUtils.loadUserAirdrops(appData);
    };
    ...
    const renderAirdrops = () => {
        ...
        if (filtered.length === 0 && airdrops.length > 0) {
            const safeFilter = AppUtils.escapeHTML(currentFilter);
            airdropTableBody.innerHTML = `<tr><td colspan="6" class="text-center py-8 text-gray-400">Tidak ada proyek dengan status "${safeFilter}".</td></tr>`;
        }
        ...
        filtered.forEach((airdrop) => {
            ...
            const safeName = AppUtils.escapeHTML(airdrop.name);
            const safeEcosystem = AppUtils.escapeHTML(airdrop.ecosystem);
            const safeWallet = AppUtils.escapeHTML(airdrop.wallet);
            const safeTasks = AppUtils.escapeHTML(airdrop.tasks);
            const safeLink = AppUtils.sanitizeURL(airdrop.link);
            const safeScreenshot = AppUtils.isValidBase64Image(airdrop.screenshot) ? airdrop.screenshot : '';

            const linkHTML = safeLink
                ? `<a href="${safeLink}" target="_blank" rel="noopener noreferrer" class="text-blue-400 hover:underline text-xs block">Visit</a>`
                : '';
            const screenshotHTML = safeScreenshot
                ? `<img src="${safeScreenshot}" class="h-10 w-10 object-cover rounded-md cursor-pointer screenshot-thumb" data-index="${index}">`
                : '<span class="text-gray-500 text-xs">-</span>';

            row.innerHTML = `<td class="px-6 py-4 font-medium text-white whitespace-nowrap">${safeName} <br> <span class="text-xs text-gray-400">${safeEcosystem}</span>${linkHTML}</td><td class="px-6 py-4">${getStatusBadge(airdrop.status)}</td><td class="px-6 py-4">${safeWallet}</td><td class="px-6 py-4">${screenshotHTML}</td><td class="px-6 py-4 text-gray-400 max-w-xs whitespace-pre-wrap break-words text-xs">${safeTasks}</td><td class="px-6 py-4 text-center"><button class="edit-btn text-yellow-400 hover:text-yellow-300 mr-3" data-index="${index}">Edit</button><button class="delete-btn text-red-400 hover:text-red-300" data-index="${index}">Hapus</button></td>`;
    ...
    const renderFilterButtons = () => {
        filterButtonsContainer.innerHTML = AppUtils.ALL_STATUSES.map(s =>
            `<button class="filter-btn ${s === 'all' ? 'active bg-blue-600 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'} py-1 px-3 rounded-full text-sm" data-filter="${AppUtils.escapeHTML(s)}">${s === 'all' ? 'Semua' : AppUtils.escapeHTML(s)}</button>`
        ).join('');
    };

    const showModal = (base64) => {
        if (AppUtils.isValidBase64Image(base64)) {
    ...
    const startEdit = (index) => {
        ...
        if (screenshotBase64 && AppUtils.isValidBase64Image(screenshotBase64)) {
    ...
    airdropForm.addEventListener('submit', (e) => {
        ...
        const existingId = editIndex !== '' ? airdrops[editIndex].id : null;
        const result = AppUtils.createAirdropData({
            name: getEl('projectName').value,
        ...
        if (editIndex !== '') {
            airdrops[editIndex] = result.data;
        } else {
            airdrops.unshift(result.data);
        }
        saveUserAirdrops();
        renderAirdrops();
        resetForm();
    ...
    airdropTableBody.addEventListener('click', (e) => {
        if (e.target.classList.contains('screenshot-thumb')) {
            ...
            if (airdrops[idx] && airdrops[idx].screenshot) {
                showModal(airdrops[idx].screenshot);
            }
        }
        ...
        if (e.target.classList.contains('delete-btn')) {
            const idx = e.target.dataset.index;
            if (airdrops[idx] && confirm(`Yakin menghapus proyek "${airdrops[idx].name}"?`)) {
